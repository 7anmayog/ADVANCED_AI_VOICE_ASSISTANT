...
import subprocess # Import subprocess for interacting with the users.
import requests # Import requests for making HTTP requests.
import keyboard # Import keyboard for simulating keyboard inputs.
import asyncio # Import asyncio for asynchronous programming.
import os # Import os for operating system function.

# Load environment variables from the .env file.
env_vars = dotenv_values(".env")
GroqAPIKey = env_vars.get("GroqAPIKey") # Retrieve the Groq API key from .env file.
...
SystemChatBot = [{"role": "system", "content": f"Hello, I am {os.environ['Username']}, You're a content writer. You have to write content like letters, articles , essays, etc."}]

# Function to perform a Google search.
def GoogleSearch(Topic):
    search(Topic) # Use pywhatkit's search function to open a file in notepad.
    return True
...
def Content(Topic):
    ...
    def OpenNotepad(File):
        default_text_editor = 'notepad.exe' # Define the default text editor.
        subprocess.Popen([default_text_editor, File]) # Default text editor.
    ...
    def ContentWriterAI(prompt):
        ...
        completion = client.chat.completions.create(
            ...
            top_p=1, #use necleus sampling for response diversiry.
        ...
        messages.append({"role": "assistant", "content": Answer}) #Append the AI response to messages.
        return Answer
    
    Topic: str = Topic.replace("content ", "") #Remove the word "content" from the topic.
...
def OpenApp(app, sess=requests.session()):
    ...
    try:
        appopen(app, match_closest=True, throw_error=True) # Attempt to open the application.
        return True # Indicate success.
...
async def TranslateAndExecute(commands: list[str]):
    ...
    for command in commands: 
        ...
        if command.startswith("open "):  # Handle "open" commands.
            ...
            print(f"NO Function Found. For {command}") # Print a error for unrecognized commands.

    results = await asyncio.gather(*funcs) # Execute all the task concurrently.
