...
#a list of system instructios for the chatbot.
SystemChatBot = [
...
try:
    with open(r"Data\Chatlog.json", "r") as f:
        messages = load(f)  # load exixting messages from the chat log.
except FileNotFoundError:
    #if the file deosn't exist, create an empty json file to store chat logs.
...
def ChatBot (Query):
    ...
    try:
        ...
        completion = client.chat.completions.create(
            model="llama3-70b-8192", # Specify the AT model to use.
            messages=SystemChatBot + [{f"role": "system", "content": RealtimeInformation()}] + messages,  # Include system instructions, real-time info, and chat history.                                            
